...
| Mobile: screens drawn by Strata | **One screen, on a desktop.** `std/ui.sta` describes a screen as an ordinary Strata table of things to draw — rectangles, text, touch regions — and paints it. `apps/orders_mobile` is the orders list against the same `Order` table the web service uses; the test asserts the display list, where taps land, and the actual pixels. Screens are **drawn, not borrowed**: asking each platform for its own widgets means two binding layers that share nothing and never match, and Apple's is not designed to be driven from C at all. One implementation, plus a shell per platform small enough to read in a sitting. |
...
| Mobile: enough to build a business app | **Not started.** Lists that scroll, text input, keyboards, navigation between screens, and a layout engine so a screen is described rather than positioned by hand. This is the work that turns one screen into an app, and it is larger than everything else in this table combined. |
